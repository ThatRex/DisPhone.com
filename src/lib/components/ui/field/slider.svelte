<script lang="ts">
	...
	$: sync.value([value], ([v]) => {
		if (value !== v) {
			if (v === default_value) navigator.vibrate?.(10)
	...
	$: displayValue = displayValueParser(value)
...
<span class="flex flex-col gap-1 grow">
	...
	<span
		class="
			...
			group border-2 rounded-md duration-75 transition
		...
		tabindex="-1"
		use:melt={$root}
		on:keydown={(e) => !['Enter', ' '].includes(e.key) || (value = default_value)}
		on:dblclick={() => {
			value = default_value
	...
	>
		...
		<span
			...
			class="
				...
				"
		/>
		<span class="ml-1.5 font-medium py-0.5 select-none">{displayValue}</span>
